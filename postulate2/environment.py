"""
postulate2.environment  --  the two-scale test bed.

Hidden 2D manifold u observed only through a fixed nonlinear 20D embedding x.
Task truth is computed from the hidden u (never from x). Two generators:
  "1A" (easy)   -- near-isometric embedding; ambient geometry recovers u.
  "1B" (hard)   -- u2 encoded only via high-frequency ALIASED terms, so no
                   static map x->u2 exists (this is why interaction is needed).
k actions (k>2 -> unit-circle coefficients => k angular sectors); optional 3rd
context u_C for continual-learning tests. Gate1Config carries all sizes/knobs.
"""
