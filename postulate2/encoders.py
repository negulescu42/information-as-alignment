"""
postulate2.encoders  --  observation -> representation (z) encoders.

Oracle2DEncoder      : true hidden u (baseline / upper bound).
PassThroughEncoder   : coords ARE the observation (feed precomputed coords).
EmergentScaleEncoder : interpolate the emergent 2D coordinate from crystallized
                       Scale-1 particles (Postulate 2, easy / static case).
build_interactive_coords[/_informed] + probe_signature : INTERACTIVE encoding --
                       build the representation by taking a few exploratory
                       actions and observing rewards. Required when the
                       observation aliases the latent (hard case, Gate 1D).
"""
