...
from environment import Gate1Config
from encoders import EmergentScaleEncoder
...
class Scale1RepresentationLearner:
    ...
    def build_particle_graph(self, particles, shuffle_signatures=False):
        ...
        # Affinity = geometry (self-tuning kNN) x behavior (gentle) x stability,
        # sparsified to the geometric kNN. Geometry carries the manifold; behavior
        # and stability are gentle organizers.
        Wfull = Wx * Wb * Wstab
        W = np.zeros_like(Wfull)
        for i in range(M):
